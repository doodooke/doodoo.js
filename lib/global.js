...
async function createIssue(
    ...
    stack,
    options = {
        gitIssueUrl: process.env.GIT_ISSUE_URL,
        zentaoIssueUrl: process.env.ZENTAO_ISSUE_URL
    }
) {
    try {
        // git issue
        if (yn(process.env.GIT_ISSUE) && options.gitIssueUrl) {
            await axios({
                url: options.gitIssueUrl,
                method: "post",
                timeout: 3000,
                headers: {
                    "Content-Type": "application/json"
    ...
    try {
        // zentao issue
        if (yn(process.env.ZENTAO_ISSUE) && options.zentaoIssueUrl) {
            const zentaoIssueUrl = new URL(options.zentaoIssueUrl);
            ...
            await axios({
                url: `${options.zentaoIssueUrl}&${sessionName}=${sessionID}`,
                method: "post",
                timeout: 3000,
