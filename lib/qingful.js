...
class Qingful extends Koa {
            ...
            this.use(async (ctx, next) => {
                ...
                if (sig && event && id) {
                    ...
                    if (message.indexOf("@pull") !== -1) {
                        await _global.execCommand(
                            process.env.GOGS_COMMAND_PULL
                        );
                    }
                    if (message.indexOf("@yarn") !== -1) {
                        await _global.execCommand(
                            process.env.GOGS_COMMAND_YARN
                        );
                    }
                    if (message.indexOf("@start") !== -1) {
                        await _global.execCommand(
                            process.env.GOGS_COMMAND_START
                        );
                    }
                    if (message.indexOf("@restart") !== -1) {
                        await _global.execCommand(
                            process.env.GOGS_COMMAND_RESTART
                        );
                    }
                    if (message.indexOf("@stop") !== -1) {
                        await _global.execCommand(
                            process.env.GOGS_COMMAND_STOP
                        );
                    }
                    ctx.body = {
                        errmsg: "ok",
    ...
    start() {
        ...
        // context
        Object.assign(this.context, qingful);
