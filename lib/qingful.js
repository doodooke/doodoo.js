            ...
            this.koa.use(async (ctx, next) => {
                ...
                if (sig && event && id) {
                    ...
                    if (!typeis(ctx.req, ["application/json"])) {
                        throw new Error(
                            "Gogs Hook ContextType Must Be application/json"
                        );
                    }
                    ...
                    if (_.startsWith(message, "@start")) {
                        _global.execCommand(process.env.GOGS_START_COMMAND);
                    }
                    if (_.startsWith(message, "@update")) {
                        _global.execCommand(process.env.GOGS_UPDATE_COMMAND);
                    }
                    if (_.startsWith(message, "@stop")) {
                        _global.execCommand(process.env.GOGS_STOP_COMMAND);
                    }
                } else {
                    await next();
                }
