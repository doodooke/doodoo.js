...
module.exports = class Router {
    ...
    getRouters() {
        this.getPatternRouters("*/controller/**/*.js", this.addonRoot);
        this.getPatternRouters();
        return this.router;
    }

    getPatternRouters(pattern = "controller/**/*.js", root = this.root) {
        const controllers = glob.sync(pattern, { cwd: root });
        for (const controller of controllers) {
            const paths = path.parse(controller);
            const Ctrl = require(path.resolve(root, controller));
            if (Ctrl instanceof KoaRouter) {
                this.router.use(
                    path.join("/", paths.dir, paths.name).replace(/\\/g, "/"),
                    Ctrl.routes()
                );
                continue;
            }
            ...
            for (const layer of layers) {
                let layerPath = path
                    ...
                    .replace(/\\/g, "/");
                // 插件
                if (root === this.addonRoot) {
                    layerPath = `/addon${layerPath.replace("/controller", "")}`;
                }
