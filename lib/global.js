...
async function createIssue(title, body) {
    ...
    if (yn(process.env.ZENTAO_ISSUE) && process.env.ZENTAO_ISSUE_URL) {
        ...
        const zentaoUrl = `${zentaoIssueUrl.origin}${zentaoIssueUrl.pathname}`;
        ...
        form.append("product", zentaoIssueUrl.searchParams.get("productID"));
        form.append("module", zentaoIssueUrl.searchParams.get("moduleID"));
        form.append("openedBuild", "trunk");
        form.append("title", title);
        form.append("steps", body);

        await axios({
            url: `${process.env.ZENTAO_ISSUE_URL}&${sessionName}=${sessionID}`,
            method: "post",
            timeout: 3000,
